...
func start_spectate(user_id: int) -> void: #should be called only when there isnt currently a GameHandler since it creates its own
	...
	game_handler.play((map_started_usec/1_000_000.0) - 0.5) #-0.5s cause im too lazy to properly account for ping and also to account for the fact that data is sent in 0.25s increments
